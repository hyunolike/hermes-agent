 ...
 * - "사실에 근거 없는 주장인가"는 세 번 좁히고도 오탐이 남았다. facts 에 있는
 *   숫자(`congestionReductionRate: 34` → "혼잡도가 34% 낮다")와 등급을 풀어 쓴
 *   표현("여유" → "한산합니다")을 근거 없는 주장으로 지적했다. 5회 실행 9건 →
 *   좁힌 뒤 4건, 그 4건도 대부분 틀렸다. 프롬프트에 이미 적힌 허용 규칙을 무시하는
 *   판정자에게 문장을 더 얹는 것은 값을 하지 않는다.
 */
...
class QualityJudge(private val provider: JudgeProvider) {
    ...
    private companion object {
        ...
        private val INSTRUCTIONS = """
            ...
            검사가 잡고 있으므로 여기서는 묻지 않는다. 아래 **하나만** 본다.

            - UNREADABLE — 문법적으로 틀린 문장("붐비는 날으로"), 한국어 문장에 섞인
              영어 단어, 번역투, 뜻이 통하지 않는 표현
            ...
            - 인용을 실제로 썼는지 여부.
            - **주장의 근거.** facts 에 있는 숫자나 인용 문서의 정책을 옮긴 문장에
              계산 근거를 더 쓰라고 요구하지 않는다. 사실 왜곡은 규칙이 잡는다.

            판단 기준:
            - 문장이 한국어로 자연스럽게 읽히면 지적하지 않는다.
            - 지적할 것이 없으면 빈 배열을 낸다. 억지로 찾지 않는다. 확신이 서지
              않으면 지적하지 않는다 — 잘못된 지적은 없는 지적보다 나쁘다.
            - 표현을 더 낫게 다듬을 수 있다는 이유로는 지적하지 않는다. 한국어로
              읽히지 않는 문장일 때만 낸다.
